...
- The quickstart at the top of the README now carries the macOS `EACCES` fix (root-owned npm prefix from the macOS Node installer; `npm config set prefix ~/.local`, no sudo). It sat only in the bottom install section, below the fold of the exact page a failing first command sends people to. Docs only; no code change.
