...
export async function profile(
  ...
  if (args.length === 0) {
    console.error("cchandoff profile: missing subcommand (add|list|remove|rename)");
    return 2;
  }
  ...
  switch (subcommand) {
    ...
    default:
      console.error(`cchandoff profile: unknown subcommand: ${subcommand}`);
      return 2;
  }
}
...
async function profileAdd(args: string[]): Promise<number> {
  if (args.length === 0 || !args[0]) {
    console.error("cchandoff profile add: missing name");
    return 2;
  }
  ...
  try {
    if (name.includes("/") || name.includes("\\") || name.includes(":")) {
      console.error(`cchandoff profile add: invalid name: ${name}`);
      return 1;
    }
    ...
    console.log(`to authenticate: cchandoff ${name} /login`);
    return 0;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`cchandoff profile add: ${msg}`);
    return 1;
  }
}
...
async function profileList(): Promise<number> {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`cchandoff profile list: ${msg}`);
    return 1;
  }
}
...
async function profileRemove(args: string[]): Promise<number> {
  if (args.length === 0 || !args[0]) {
    console.error("cchandoff profile remove: missing name");
    return 2;
  }
  ...
  try {
    ...
    if (!config.profiles[name]) {
      console.error(`cchandoff profile remove: profile not found: ${name}`);
      return 1;
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`cchandoff profile remove: ${msg}`);
    return 1;
  }
}
...
async function profileRename(args: string[]): Promise<number> {
  if (args.length < 2 || !args[0] || !args[1]) {
    console.error("cchandoff profile rename: missing old or new name");
    return 2;
  }
  ...
  try {
    ...
    if (!config.profiles[oldName]) {
      console.error(`cchandoff profile rename: profile not found: ${oldName}`);
      return 1;
    }

    if (newName.includes("/") || newName.includes("\\") || newName.includes(":")) {
      console.error(`cchandoff profile rename: invalid name: ${newName}`);
      return 1;
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`cchandoff profile rename: ${msg}`);
    return 1;
  }
}
