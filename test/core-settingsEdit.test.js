...
import { installHooks, uninstallHooks, } from "../src/core/settingsEdit.js";
...
test("settingsEdit: installHooks is idempotent", () => {
    ...
    installHooks(configDir, { sessionStartCmd: cmd });
    const afterSecond = readFileSync(join(configDir, "settings.json"), "utf8");
...
test("settingsEdit: uninstallHooks removes warmswap hooks", () => {
    ...
    if (sessionStart) {
        assert.equal(sessionStart?.length ?? 0, 0);
    }
});
