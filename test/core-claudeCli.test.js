...
test("claudeCli: claudePath respects env override", () => {
    const orig = process.env.WARMSWAP_CLAUDE_BIN;
    process.env.WARMSWAP_CLAUDE_BIN = "/fake/path/claude";
    const path = claudePath();
    assert.equal(path, "/fake/path/claude");
    process.env.WARMSWAP_CLAUDE_BIN = orig;
});
test("claudeCli: claudePath returns default when no override", () => {
    const orig = process.env.WARMSWAP_CLAUDE_BIN;
    delete process.env.WARMSWAP_CLAUDE_BIN;
    const path = claudePath();
