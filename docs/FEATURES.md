...
| **Same-account refresh (/clear + auto-inject)** | `refresh` command (prints 2-step flow) | `/refresh` skill (compose and tell user to `/clear`) | advisor copy names refresh as the cold-cache path ✅ | "Refresh In Place…" menu action → `warmswap refresh` |
| **Trail mode (continuous capture)** | `trail on/off/status` | rules + skill maintain `.claude/handoff/trail.md` + advisor staleness nudge | advisor nudge ✅; `/trail` skill controls it | "Trail Mode: toggle" menu action wires `warmswap trail on/off` |
