...
| **How much you saved (history)** | `audit` / `audit --json` per-event + totals + per-class breakdown | — (post-hoc metric) | — | savings totals in popover with switch/refresh/post-reset breakdown (from `audit --json`) |
