...
| Project | What it does | Gap vs cchandoff |
|---|---|---|
...
- **ccusage** (npm, very popular): parses the same `projects/**/*.jsonl` files, daily/weekly/session/blocks reports, cost estimates from token buckets. Reads `~/.claude` and `~/.config/claude`; env var accepts comma-separated dirs. Complementary — cchandoff links to it for deep reporting rather than reimplementing analytics; cchandoff's own math is narrowly scoped to window-burn + switch-tax estimates.
...
cchandoff = the **workflow tool**: profiles (isolated `CLAUDE_CONFIG_DIR` accounts) + deterministic transcript snapshots + hook-automated capture/rehydration + switch-tax measurement + an honest explainer of the cache physics. It deliberately does not compete with ccusage (analytics), cc-switch (provider configs), or in-repo handoff skills (it can coexist; its own skill covers that niche). Nothing in the ecosystem combines these layers today.
