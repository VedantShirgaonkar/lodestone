...
**Practical rule for lodestone:** estimate window burn as
...
- The same math explains why resuming a stale (>1h idle) giant session **on the same account** is expensive: full re-write at 2×. lodestone's snapshot/rehydrate therefore also helps single-account users — bigger OSS audience.
