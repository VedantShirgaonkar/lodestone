...
4. **Command titles are searchable**: keep them descriptive ("lodestone: Handoff & Switch Account") — already done.
...
All listing copy is written around a placeholder token so the user's final name choice (shortlist verified free on npm + marketplace 2026-07-13: lodestone · batonpass · contextcarry; ccswap dropped — existing juanmackie/ccswap project) drops in with one sweep. Nothing publishes before explicit user confirmation of the name.
