...
3. **Integration tests drive the built CLI as a child process** (`node dist/cli.js …` via `execFile`) with env `{HOME, XDG_CONFIG_HOME, CLAUDE_CONFIG_DIR, CCHANDOFF_CLAUDE_BIN}` pointing into per-test temp dirs, `cwd` set to a temp project dir containing a `.git` marker directory. Never mutate the parent process env; never touch the real `~/.claude`.
...
## `cchandoff snapshot [--session <id>] [--out <path>] [--quiet] [--json]`
...
## `cchandoff handoff [--distill] [--force] [--session <id>] [--json]`
...
## `cchandoff switch <profile> [--distill] [--force] [--stay] [--json]`
...
3. Compute and print the comparison — every value computed, none hardcoded:
   ...
   ```
4. `--stay`: stop here (exit 0). Otherwise `launchInteractive(target, [], {cwd})` and propagate the exit code. (No hook exists yet in Phase 2, so also print: `tip: paste .claude/handoff/latest.md into the new session, or run cchandoff init (Phase 3) for automatic injection` — remove this line in Phase 3.)
...
```
- Window line from `windowBurn(configDir, now)`: expired/empty → `no recent activity`; else bar = burn vs `asPctOfWindow(burn, plan)` (plan from cchandoff config `settings.plan`, default `pro`; print `est` always).
...
1. `claude` resolvable (CCHANDOFF_CLAUDE_BIN honored) and `--version` parses to ≥ 2.0.0.
2. cchandoff config parses; ≥1 profile; every profile's configDir exists.
3. Per profile: login hint available (`oauthAccount` present) — missing = FAIL with `run: cchandoff login <name>`.
...
- handoff: `--distill` cold-cache refusal (plant transcript with old lastTs → exit 1, message mentions minutes + --force); `--force` path with `CCHANDOFF_CLAUDE_BIN` fake that emits a fixed distilled markdown → assert merge (narrative replaced, Files section deterministic, meta.distilled true); fake-claude failure → deterministic handoff survives, exit 0.
