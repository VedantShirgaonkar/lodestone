...
2. **`warmswap dash` (now):** live full-screen TUI (ANSI, zero deps, 1–2s refresh): per-profile real quota bars with resets and pacing, live sessions' context sizes + 1h-cache countdowns, switch-tax panel, advisor line, keepalive status. This IS the screenshot's popover, terminal-native, and works for both profiles.
...
- The extension ships only after the CLI is published (it depends on a global `warmswap` install) — keeps launch scope honest.
