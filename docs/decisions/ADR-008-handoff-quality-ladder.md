...
2. **Tier 2: `cchandoff handoff --distill`** — after leaving the session but within the cache TTL; resume-fork distillation, cold-cache guard (>55min idle refuses without `--force`).
