...
## Decision
`warmswap switch <target> --keep-warm <duration>` (and standalone `warmswap keepalive <profile> [--for <duration>]`):
...
## Consequences
- Scheduling: v1 uses a detached local process with a pidfile under `~/.config/warmswap/` (no daemons, no cron edits); survives only while the machine is awake — documented.
