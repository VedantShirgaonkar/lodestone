...
## Decision
`lodestone switch <target> --keep-warm <duration>` (and standalone `lodestone keepalive <profile> [--for <duration>]`):
...
## Consequences
- Scheduling: v1 uses a detached local process with a pidfile under `~/.config/lodestone/` (no daemons, no cron edits); survives only while the machine is awake — documented.
