...
## Decision
- **TypeScript**, compiled with `tsc` to ESM in `dist/`; published to npm as `cchandoff` with bins `cchandoff` and `cch`.
...
- `npx cchandoff` works out of the box; no build step for users.
