...
## Decision
- **TypeScript**, compiled with `tsc` to ESM in `dist/`; published to npm as `warmswap` with bins `warmswap` and `cch`.
...
- `npx warmswap` works out of the box; no build step for users.
