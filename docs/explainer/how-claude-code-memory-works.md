...
Plus the compounding effect: the naive path drags C into every later turn on B; the handoff path starts a lean session. That's the entire thesis of lodestone — and the same mechanism rescues single-account users resuming cold sessions.
