...
## What cchandoff writes

- **Handoff files**: `.claude/handoff/latest.md` and archived versions (your project directory)
- **Config**: `~/.config/cchandoff/config.json` (profiles registry, settings)
- **Logs**: `~/.config/cchandoff/cchandoff.log` (command history, errors; size-capped, rotated; no sensitive data)
- **Usage cache**: `~/.config/cchandoff/usage-cache.json` (quota % and reset times from statusline; no tokens or credentials)
- **Hook state**: Advisor debounce tracker in `<configDir>/cchandoff/` (one-per-5%-step per session)
- **Keepalive schedule**: PID file in `~/.config/cchandoff/keepalive/` (process bookkeeping, no credentials)

## What cchandoff never does
...
- **Logs**: Plain text in `~/.config/cchandoff/cchandoff.log`, readable by the user
